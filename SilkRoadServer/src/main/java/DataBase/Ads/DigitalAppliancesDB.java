...
import ADS.DigitalGoods.DigitalAppliance;
...
public class DigitalAppliancesDB
{
    ...
    synchronized public static int insertDigitalAppliances (JSONObject json) throws SQLException {
        setTime();
        ...
        statement.setString(10, "DigitalAppliances");
    ...
    synchronized public static List<String> selectDigitalAppliances (int from, int to) throws SQLException {
        setTime();
        connection = DriverManager.getConnection(url);
        List<String> jsons = new ArrayList<>();

        String sql = "SELECT * FROM (select title, image, price, city, description, ownerID, brand, date, isNew, adID" +
                " from Ads WHERE category = \"DigitalAppliances\") s LIMIT " + from + ", " + to + ";";
        try  {
    ...
    synchronized public static List<String> selectDigitalAppliancesByPrice (String lowerBound, String upperBound, int from, int to) throws SQLException {
        setTime();
        connection = DriverManager.getConnection(url);
        List<String> jsons = new ArrayList<>();

        String sql = "SELECT * FROM(SELECT title, image, price, city, description, ownerID, brand, date, isNew, adID" +
                " FROM Ads WHERE category = \"DigitalAppliances\" And Price BETWEEN " + lowerBound + " AND " + upperBound + ") s LIMIT " +
                 from + ", " + to + ";";
        try  {
            Statement statement = connection.createStatement();
            ResultSet results = statement.executeQuery(sql);
    ...
    synchronized public static List<String> selectDigitalAppliancesByCity (String inputCity, int from, int to) throws SQLException {
        setTime();
        connection = DriverManager.getConnection(url);
        List<String> jsons = new ArrayList<>();

        String sql = "SELECT * FROM (select title, image, price, city, description, ownerID, brand, date, isNew, adID" +
                " from Ads WHERE category = \"DigitalAppliances\" AND city = \"" + inputCity +
                "\") s LIMIT " + from + ", " + to + ";";
        try  {
            Statement statement = connection.createStatement();
            ResultSet results = statement.executeQuery(sql);
